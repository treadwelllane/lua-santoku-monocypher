...
test("sign_request and verify_request", function ()
  ...
  local ts, sig = id:sign_request(body)
  assert(type(ts) == "number")
  assert(type(sig) == "string")
  local ok, err = crypto.verify_request(id:public_key(), sig, id:sub(), ts, body)
  assert(ok == true, err)
end)
...
test("verify_request wrong signature fails", function ()
  local id = crypto.derive_identity("test-secret")
  local body = "request body"
  local ts, _ = id:sign_request(body)
  local id2 = crypto.derive_identity("other-secret")
  local _, sig2 = id2:sign_request(body)
  local ok, errmsg = crypto.verify_request(id:public_key(), sig2, id:sub(), ts, body)
  assert(ok == nil)
